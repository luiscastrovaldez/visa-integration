...
public class VisaXmlParserServiceImplTest {
	...
	public void testParseVisaOperationResponseXml() {
		...
		assertEquals("2000601061101112270844150156", respuestaVisa.getPedido()
				.getEticket());
		assertEquals(1, respuestaVisa.getPedido().getOperacion().getId());
		assertEquals(5, respuestaVisa.getPedido().getOperacion().getCampos()
				.size());
	...
	public void testParseVisaETicketResponseXml() {
		...
		assertEquals(VisaIntegrationConstants.CAMPO_COD_TIENDA, respuestaVisa
				.getRegistro().get(0).getId());
		assertEquals(VisaIntegrationConstants.CAMPO_NUM_ORDEN, respuestaVisa
				.getRegistro().get(1).getId());
		assertEquals(VisaIntegrationConstants.CAMPO_E_TICKET, respuestaVisa
				.getRegistro().get(2).getId());
		assertEquals("998877601", respuestaVisa.getRegistro().get(0).getValue());
		assertEquals("699", respuestaVisa.getRegistro().get(1).getValue());
		assertEquals("2000601061101112270844150156", respuestaVisa
				.getRegistro().get(2).getValue());
		assertEquals(1, respuestaVisa.getMensajes().size());
		assertEquals("1", respuestaVisa.getMensajes().get(0).getId());
	}
}
